...
public class YouTubeUtility {
	...
	public static String getInfoString(String id) throws IOException {
		...
		//Log.i("ffa", "网址"+YoutubeUrlIDs.YOUTUBE_VIDEO_INFORMATION_URL + id + "&el=embedded&gl=US&hl=en&eurl=" + URLEncoder.encode("https://youtube.googleapis.com/v/" + id, "UTF-8") + "&asv=3&sts=1588");
		HttpResponse lResp = null;
		lResp = lClient.execute(lGetMethod);
		ByteArrayOutputStream lBOS = new ByteArrayOutputStream();
		String lInfoStr = null;
	...
	public static ArrayList<Video> func_get_video_script1(String web_source) throws Exception {
		...
		for (int i = 0; i < lArgs.length; i++) {
			...
			if (lArgValStrArr != null) {
				...
				Log.i("keydata", "key"+i+lArgValStrArr[0]);
				Log.i("keydata", "value"+i+lArgValStrArr[1]);
				Log.i("ksak1", ""+URLDecoder.decode(lArgValStrArr[1]));
			}
		}
		//Find out the URI string from the parameters
		//Populate the list of formats for the video
		String lFmtList = URLDecoder.decode(lArgMap.get("fmt_list"));
		String lFmtList1 = URLDecoder.decode(lArgMap.get("iurlmaxres"));
		Log.i("ksak", ""+lFmtList1);
		Log.i("keydata", "key"+lFmtList);
		...
		if (null != lStreamList) {
			...
			for (String lStreamStr : lStreamStrs) {
				...
				Log.i("keydata", "keysize"+lStreams.size());
			}
			Log.i("keydata", "keysize"+lStreams.size());
			...
			for (Format lformat : lFormats) {
				Log.i("keydata", "keydata"+lFormats.size());
				Video lvid = new Video();
				int lIndex = lFormats.indexOf(lformat);
				int id = lformat.getId();
				Log.i("keydata", "lIndex"+lIndex);
				Log.i("keydata", "id"+id);
				//get video quality type string..
				String ltype = typeMap.get(id);
				if ((lIndex >= 0) &&(lStreams.size()>lIndex)) {
					VideoStream lSearchStream = lStreams.get(lIndex);
					String lUriString = Uri.decode(Uri.parse(lSearchStream.getUrl()).toString());
					Log.i("keydata", "ltype"+ltype);
					Log.i("AAA", "lUriString"+lUriString);
					lvid.type = ltype;
					
					lvid.url = lUriString;
					if (lvid.type != null && lvid.url != null) {
						video_list.add(lvid);
						Log.i("keydata", " 列表大小"+video_list.size());
					}
				}
				Log.i("keydata", " 列表大小1"+video_list.size());
			}
			
			
			Log.i("keydata", " 列表大小2"+video_list.size());
		}
		Log.i("keydata", " 列表大小3"+video_list.size());
		return video_list;
	}
